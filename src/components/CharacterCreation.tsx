...
import { Sword, Wand2, Shield, Flame, Sparkles, Sun, Zap, Heart, ShieldAlert, User, Check, Play } from 'lucide-react';
...
export const CharacterCreation: React.FC<CharacterCreationProps> = ({ onStartAdventure, onContinueAdventure, hasSavedGame }) => {
  ...
  const calculatedStats = {
    hp: selectedClass.baseStats.hp + selectedRace.bonuses.hp,
    mp: selectedClass.baseStats.mp + selectedRace.bonuses.mp,
    atk: selectedClass.baseStats.atk + selectedRace.bonuses.atk,
    def: selectedClass.baseStats.def + selectedRace.bonuses.def,
    spd: selectedClass.baseStats.spd + selectedRace.bonuses.spd,
    crit: selectedClass.baseStats.crit + selectedRace.bonuses.crit,
  };
      ...
      logs: [
        {
          ...
          category: 'system',
          title: '冒険開始',
          description: `【${heroName.trim() || '無名の冒険者'}】（${selectedRace.name} / ${selectedClass.name}）が元素の迷宮へと一歩を踏み出した。`,
        },
      ],
    };
            ...
            return (
              <div
                ...
                className={`cursor-pointer rounded-xl p-4 transition-all border relative flex flex-col justify-between ${
                  isSelected
                    ? 'bg-gradient-to-b from-indigo-900/80 to-slate-900 border-indigo-400 shadow-lg shadow-indigo-950/50 ring-2 ring-indigo-500/50'
          ...
          {MAGIC_TYPES.map((magic) => {
            ...
            return (
              <div
                ...
                className={`cursor-pointer rounded-xl p-4 transition-all border relative flex flex-col justify-between ${
                  isSelected
                    ? 'bg-gradient-to-b from-purple-900/80 to-slate-900 border-purple-400 shadow-lg shadow-purple-950/50 ring-2 ring-purple-500/50'
                    : 'bg-slate-900/60 border-slate-800 hover:border-slate-700 hover:bg-slate-900/90'
                }`}
              >
                {isSelected && (
